...
Pipe::realize() {
  ...
  if (opts.input_type == Option::NONE) {
    decoder = new NoneDecoder(opts, ctx, model, constraint, NP, deprels_alphabet.size());
  } else if (opts.input_type == Option::FULL) {
    decoder = new FullDecoder(opts, ctx, model, constraint, NP, deprels_alphabet.size());
  } else if (opts.input_type == Option::PARTIAL) {
    decoder = new PartialDecoder(opts, ctx, model, constraint, NP, deprels_alphabet.size());
  } else {
